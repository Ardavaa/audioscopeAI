...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('audioscopeai-api-ready', function(event) {
        isAPIReady = true;
        console.log('API is ready for predictions');
        updateUIBasedOnAPIStatus(true);
    });
    
    document.addEventListener('audioscopeai-api-error', function(event) {
        isAPIReady = false;
    ...
    async function handleFileUpload(file) {
        ...
        try {
            ...
            await window.AudioscopeAIAPI.predictRespiratoryDisease(file, null);
        } catch (error) {
    ...
    async function startProcessing() {
        ...
        try {
            ...
            const result = await window.AudioscopeAIAPI.predictRespiratoryDisease(uploadedFile);
    ...
    function updateResultsUI(result) {
        ...
        const confidencePercentage = window.AudioscopeAIAPI.formatConfidence(result.confidence);
        if (confidenceValue) confidenceValue.textContent = confidencePercentage;

        const confidenceLevel = window.AudioscopeAIAPI.getConfidenceLevel(result.confidence);
        ...
        const recommendations = window.AudioscopeAIAPI.getHealthRecommendations(result.prediction, result.confidence);
    ...
    function updateProbabilityDisplay(probabilities) {
        ...
        const formatted = window.AudioscopeAIAPI.formatProbabilities(probabilities);
    ...
    function updateAIInsights(aiInsights) {
        ...
        if (confidenceInterpretationElement) {
            ...
            if (!confidenceText && currentPredictionResult) {
                const confidenceLevel = window.AudioscopeAIAPI.getConfidenceLevel(currentPredictionResult.confidence);
        ...
        downloadReportBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                try {
                    ...
                    const confidence = window.AudioscopeAIAPI.formatConfidence(currentPredictionResult.confidence);
                    const confidenceLevel = window.AudioscopeAIAPI.getConfidenceLevel(currentPredictionResult.confidence);
                    const recommendations = window.AudioscopeAIAPI.getHealthRecommendations(
...
Model Version: ${window.AudioscopeAIAPI?.apiStatus()?.version || 'Unknown'}
...
===
Generated by Audioscope AI Analysis System
${new Date().toISOString()}

© 2024 Audioscope AI - AI-Powered Respiratory Health Analysis
                    `;
                    ...
                    link.download = `audioscope-ai-analysis-${predictionName}-${timestamp}.txt`;
